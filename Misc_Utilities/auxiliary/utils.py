...
def translate(names, dic=None):
    ...
    if isinstance(names, list) or isinstance(names, tuple):
        # preserves order
        new_names = list(names).copy()
        for i in range(len(names)):
            if names[i] in dic.keys():
                new_names[i] = dic[names[i]]
            elif names[i] in inv_dic.keys():
                new_names[i] = dic[names[i]]
        ...
        return {dic.get(n, n) if n in dic.keys() else inv_dic.get(n, n) for n in names}

    elif isinstance(names, str):
        n = names
        return dic.get(n, n) if n in dic.keys() else inv_dic.get(n, n)

    elif isinstance(names, dict):
        new_names = names.copy()
        for k, v in names.items():
            if k in dic.keys():
                new_names[dic[k]] = names[k]
                del new_names[k]
            elif k in inv_dic.keys():
                new_names[inv_dic[k]] = names[k]
                del new_names[k]
        return new_names
...
def compose(dic1, dic2):
    ...
    composition = {}
    for k in dic1.keys():
        if dic1[k] in dic2.keys():
            composition[k] = dic2[dic1[k]]
        else:
            composition[k] = dic1[k]
    for k in dic2.keys():
        if k in dic1.values():
            continue
        else:
            composition[k] = dic2[k]
    return composition