import sys
sys.path.append('../HARK_SSJ_example/Misc_Utilities')
...
from auxiliary.utils import translate, compose
# from sequence_jacobian_files import asymptotic
import asymptotic
from sequence_jacobian.blocks.block import Block
from sequence_jacobian.blocks.het_block import HetBlock

from sequence_jacobian.classes import  ImpulseDict,SteadyStateDict

import copy
from sequence_jacobian.utilities.bijection import Bijection


''' Part 0: Exogenous blocks: exog jac and exog td '''

def exog(jac, td,het_func):
    return ExogBlock(jac, td,het_func)

class ExogBlock(Block):

    def __init__(self, exog_jac, td,het_func):
        
        Block.__init__(self)
        self.exog_jac = exog_jac
        self.exog_td = td
        self.input_list = utils.input_list(td)
        self.output_list = utils.output_list(td)
        ...
        self.het_func = het_func
        self.jacobian_options = self.input_defaults_smart('_jacobian')
        self.M = Bijection({})

    def __repr__(self):
        return f"<ExogBlock '{self.exog_td.__name__}'>"
    ...
    def remap(self, dic):
        self.dic = compose(self.dic, dic)
        self.input_list = translate(self.input_list, dic)
        self.output_list = translate(self.output_list, dic)
        self.inputs = translate(self.inputs, dic)
        self.outputs = translate(self.outputs, dic)
    ...
    def td(self, ss, **kwargs):
        ...
        for k, v in kwargs.items():
            ...
            kwargs_new[translate(k, self.dic)] = v  # Displace(v, ss=ss.get(k, None), name=k)
            if T is None:
                T = v.size
            else:
                assert T == v.size
        ...
        for k in self.input_list:
            if translate(k, self.dic) not in kwargs_new:
                kwargs_new[translate(k, self.dic)] = ss[k] * np.ones(T)

        return dict(zip(self.output_list, utils.make_tuple(self.exog_td(**kwargs_new))))

    # def jac(self, ss, T=None, shock_list=None, h=None):
    #     if self.dic is not None:
    #         exog_jac_here = {translate(k, self.dic): translate(self.exog_jac[k], self.dic) for k in self.exog_jac.keys()}
    #         return exog_jac_here
    #     else:
    #         return self.exog_jac
        
    
    # def partial_jacobians(self):
    ...
    def _impulse_nonlinear(self, ssin, inputs, outputs, internals, monotonic=False):
        ...
        """
        # inps= {k: ss[k] + v[t, ...] for k, v in inputs.items()}
    
    
        inps= {k: ss[k] + v for k, v in inputs.items()}

        aggregates = {'C': self.het_func(inps['Z'])[0], 
                      'A': self.het_func(inps['Z'])[1]}
        ...
        internals_dict = {self.name: {'C' :self.het_func(inps['Z'])[0] , 'A':  self.het_func(inps['Z'])[1]}  }
        """
        return ImpulseDict(aggregates, internals_dict, inputs.T) - ssin
        """
        return ImpulseDict(aggregates) - ssin
...
class SimpleBlock:
    ...
    def clone(self, nclones, clone_vars):
        clones = list()
        for i in range(nclones):
            new_block = copy.copy(self)
            new_names = {var: var + str(i) for var in clone_vars}
            new_block.remap(new_names)
    ...
    def td(self, ss, **kwargs):
        ...
        for k, v in kwargs.items():
            if np.isscalar(v):
                raise ValueError(f'Keyword argument {k}={v} is scalar, should be time path.')
            kwargs_new[translate(k, self.dic)] = Displace(v, ss=ss.get(k, None), name=k)

        for k in self.input_list:
            if translate(k, self.dic) not in kwargs_new:
                kwargs_new[translate(k, self.dic)] = Ignore(ss[k])
    ...
    def jac(self, ss, T=None, shock_list=None, h=1E-5):
        ...
        x_ss_new = {translate(k, self.dic): Ignore(ss[k]) for k in self.input_list}
        ...
        for k in shock_list:
            ...
            x_ss_new[translate(k, self.dic)] = reporter
            self.f(**x_ss_new)
            ...
            for i in relevant_displacements:
                ...
                x_ss_new[translate(k, self.dic)] = Perturb(ss[k], h, i)
            ...
            x_ss_new[translate(k, self.dic)] = Ignore(ss[k])
